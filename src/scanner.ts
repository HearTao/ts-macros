...
import { CharacterCodes, TokenFlags } from "./types";
import {
...
import { parsePseudoBigInt, nodeIsMissing } from "./utils";
...
function isJSDocTypeExpressionOrChild(node: Node): boolean {
  return (
    ...
    (node.parent && isJSDocTypeExpressionOrChild(node.parent))
  );
}
...
export function getTextOfNodeFromSourceText(
  ...
  let text = sourceText.substring(
    includeTrivia ? node.pos : skipTrivia(sourceText, node.pos),
    node.end
  );
...
export function computePositionOfLineAndCharacter(
  ...
  if (line < 0 || line >= lineStarts.length) {
    if (allowEdits) {
      ...
      line =
        line < 0 ? 0 : line >= lineStarts.length ? lineStarts.length - 1 : line;
    } else {
      Debug.fail(
...
export function createScanner(
  ...
  return {
    ...
    hasExtendedUnicodeEscape: () =>
      (tokenFlags & TokenFlags.ExtendedUnicodeEscape) !== 0,
    hasPrecedingLineBreak: () =>
      (tokenFlags & TokenFlags.PrecedingLineBreak) !== 0,
    isIdentifier: () =>
  ...
  function scanNumber(): { type: SyntaxKind; value: string } {
    ...
    if (
      ...
      if (!finalFragment) {
        ...
        scientificFragment =
          text.substring(end, preNumericPart) + finalFragment;
        end = pos;
      }
    }
    let result: string;
  ...
  function scanTemplateAndSetTokenValue(): SyntaxKind {
    const startedWithBacktick =
      text.charCodeAt(pos) === CharacterCodes.backtick;
      ...
      switch (ch) {
        ...
        case CharacterCodes.openParen:
          ...
          if (text.charCodeAt(pos) === CharacterCodes.closeParen) {
            tokenValue = "12";
            return pos++, (token = SyntaxKind.StringLiteral);
          }
        ...
        case CharacterCodes.backslash:
          ...
          if (
            ...
            pos += 6;
            tokenValue =
              String.fromCharCode(cookedChar) + scanIdentifierParts();
        ...
        default:
          if (isIdentifierStart(ch, languageVersion)) {
            ...
            while (
              pos < end &&
              isIdentifierPart((ch = text.charCodeAt(pos)), languageVersion)
            )
              pos++;
    ...
    if (tokenIsIdentifierOrKeyword(token)) {
      ...
      while (pos < end) {
        ...
        if (
          ...
          (firstCharPosition === pos
            ? isIdentifierStart(ch, languageVersion)
  ...
  function scanJSDocToken(): JsDocSyntaxKind {
    ...
    if (isIdentifierStart(ch, ScriptTarget.Latest)) {
      while (
        isIdentifierPart(text.charCodeAt(pos), ScriptTarget.Latest) &&
        pos < end
      ) {
        pos++;
      }
