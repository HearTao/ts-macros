...
import { some, createMap, toArray, map, last, lastOrUndefined } from "./core";
import {
  objectAllocator,
  isNodeKind,
  forEach,
...
declare module "typescript" {
  ...
  export interface Identifier extends PrimaryExpression, Declaration {
    ...
    >; // Only defined on synthesized nodes. Though not syntactically valid, used in emitting diagnostics, quickinfo, and signature help.
...
export function createNode(kind: SyntaxKind, pos?: number, end?: number): Node {
  if (kind === SyntaxKind.SourceFile) {
    ...
    return new (NodeConstructor ||
      (NodeConstructor = objectAllocator.getNodeConstructor()))(kind, pos, end);
  }
}
...
export function forEachChild<T>(
  ...
  switch (node.kind) {
    case SyntaxKind.QualifiedName:
      return (
        visitNode(cbNode, (<QualifiedName>node).left) ||
    ...
    case SyntaxKind.ShorthandPropertyAssignment:
      return (
        ...
        visitNode(
          cbNode,
          (<ShorthandPropertyAssignment>node).exclamationToken
        ) ||
        visitNode(cbNode, (<ShorthandPropertyAssignment>node).equalsToken) ||
        visitNode(
          cbNode,
          (<ShorthandPropertyAssignment>node).objectAssignmentInitializer
        )
      );
    ...
    case SyntaxKind.IndexSignature:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<SignatureDeclaration>node).typeParameters
        ) ||
    ...
    case SyntaxKind.ArrowFunction:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<FunctionLikeDeclaration>node).typeParameters
        ) ||
        visitNodes(
          cbNode,
          cbNodes,
          (<FunctionLikeDeclaration>node).parameters
        ) ||
    ...
    case SyntaxKind.ImportType:
      return (
        visitNode(cbNode, (<ImportTypeNode>node).argument) ||
    ...
    case SyntaxKind.TypeOperator:
      return visitNode(
        cbNode,
        (<ParenthesizedTypeNode | TypeOperatorNode>node).type
      );
    case SyntaxKind.IndexedAccessType:
      return (
        visitNode(cbNode, (<IndexedAccessTypeNode>node).objectType) ||
    ...
    case SyntaxKind.TaggedTemplateExpression:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<TaggedTemplateExpression>node).typeArguments
        ) ||
        visitNode(cbNode, (<TaggedTemplateExpression>node).template)
      );
    case SyntaxKind.TypeAssertionExpression:
      return (
        visitNode(cbNode, (<TypeAssertion>node).type) ||
    ...
    case SyntaxKind.YieldExpression:
      return (
        visitNode(cbNode, (<YieldExpression>node).asteriskToken) ||
    ...
    case SyntaxKind.BinaryExpression:
      return (
        visitNode(cbNode, (<BinaryExpression>node).left) ||
        visitNode(cbNode, (<BinaryExpression>node).operatorToken) ||
        visitNode(cbNode, (<BinaryExpression>node).right)
      );
    case SyntaxKind.AsExpression:
      return (
        visitNode(cbNode, (<AsExpression>node).expression) ||
    ...
    case SyntaxKind.SourceFile:
      return (
        visitNodes(cbNode, cbNodes, (<SourceFile>node).statements) ||
    ...
    case SyntaxKind.IfStatement:
      return (
        visitNode(cbNode, (<IfStatement>node).expression) ||
        visitNode(cbNode, (<IfStatement>node).thenStatement) ||
        visitNode(cbNode, (<IfStatement>node).elseStatement)
      );
    case SyntaxKind.DoStatement:
      return (
        visitNode(cbNode, (<DoStatement>node).statement) ||
        visitNode(cbNode, (<DoStatement>node).expression)
      );
    case SyntaxKind.WhileStatement:
      return (
        visitNode(cbNode, (<WhileStatement>node).expression) ||
    ...
    case SyntaxKind.ForInStatement:
      return (
        visitNode(cbNode, (<ForInStatement>node).initializer) ||
    ...
    case SyntaxKind.WithStatement:
      return (
        visitNode(cbNode, (<WithStatement>node).expression) ||
        visitNode(cbNode, (<WithStatement>node).statement)
      );
    case SyntaxKind.SwitchStatement:
      return (
        visitNode(cbNode, (<SwitchStatement>node).expression) ||
    ...
    case SyntaxKind.CaseClause:
      return (
        visitNode(cbNode, (<CaseClause>node).expression) ||
    ...
    case SyntaxKind.LabeledStatement:
      return (
        visitNode(cbNode, (<LabeledStatement>node).label) ||
    ...
    case SyntaxKind.ClassExpression:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<ClassLikeDeclaration>node).typeParameters
        ) ||
        visitNodes(
          cbNode,
          cbNodes,
          (<ClassLikeDeclaration>node).heritageClauses
        ) ||
    ...
    case SyntaxKind.InterfaceDeclaration:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<InterfaceDeclaration>node).typeParameters
        ) ||
    ...
    case SyntaxKind.TypeAliasDeclaration:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<TypeAliasDeclaration>node).typeParameters
        ) ||
    ...
    case SyntaxKind.EnumMember:
      return (
        visitNode(cbNode, (<EnumMember>node).name) ||
    ...
    case SyntaxKind.ImportClause:
      return (
        visitNode(cbNode, (<ImportClause>node).name) ||
    ...
    case SyntaxKind.NamedExports:
      return visitNodes(
        cbNode,
        cbNodes,
        (<NamedImportsOrExports>node).elements
      );
    ...
    case SyntaxKind.ExportSpecifier:
      return (
        visitNode(cbNode, (<ImportOrExportSpecifier>node).propertyName) ||
    ...
    case SyntaxKind.TemplateExpression:
      return (
        visitNode(cbNode, (<TemplateExpression>node).head) ||
        visitNodes(cbNode, cbNodes, (<TemplateExpression>node).templateSpans)
      );
    case SyntaxKind.TemplateSpan:
      return (
        visitNode(cbNode, (<TemplateSpan>node).expression) ||
    ...
    case SyntaxKind.ExpressionWithTypeArguments:
      return (
        visitNode(cbNode, (<ExpressionWithTypeArguments>node).expression) ||
        visitNodes(
          cbNode,
          cbNodes,
    ...
    case SyntaxKind.JsxOpeningElement:
      return (
        ...
        visitNodes(
          cbNode,
          cbNodes,
          (<JsxOpeningLikeElement>node).typeArguments
        ) ||
    ...
    case SyntaxKind.JsxAttribute:
      return (
        visitNode(cbNode, (<JsxAttribute>node).name) ||
    ...
    case SyntaxKind.JsxExpression:
      return (
        visitNode(cbNode, (node as JsxExpression).dotDotDotToken) ||
    ...
    case SyntaxKind.JSDocFunctionType:
      return (
        visitNodes(cbNode, cbNodes, (<JSDocFunctionType>node).parameters) ||
    ...
    case SyntaxKind.JSDocTypedefTag:
      return (
        ...
        (node as JSDocTypedefTag).typeExpression!.kind ===
    ...
    case SyntaxKind.JSDocEnumTag:
      return (
        ...
        visitNode(
          cbNode,
          (node as JSDocReturnTag | JSDocTypeTag | JSDocThisTag | JSDocEnumTag)
            .typeExpression
        )
      );
...
export function parseIsolatedJSDocComment(
  ...
  const result = Parser.JSDocParser.parseIsolatedJSDocComment(
    content,
    start,
    length
  );
...
export function parseJSDocTypeExpressionForTests(
  ...
  return Parser.JSDocParser.parseJSDocTypeExpressionForTests(
    content,
    start,
    length
  );
...
namespace Parser {
  ...
  const disallowInAndDecoratorContext =
  ...
  export function parseIsolatedEntityName(
    ...
    const isInvalid =
      token() === SyntaxKind.EndOfFileToken && !parseDiagnostics.length;
    clearState();
  ...
  export function parseJsonText(
    ...
    sourceFile.flags = (contextFlags as any) as ts.NodeFlags;
    ...
    if (token() === SyntaxKind.EndOfFileToken) {
      ...
      switch (token()) {
        ...
        case SyntaxKind.StringLiteral:
          if (lookAhead(() => nextToken() !== SyntaxKind.ColonToken)) {
            statement.expression = parseLiteralNode() as
              | StringLiteral
              | NumericLiteral;
            break;
          }
      ...
      sourceFile.endOfFileToken = parseExpectedToken(
  ...
  function parseSourceFileWorker(
    ...
    sourceFile.flags = contextFlags as any;
    ...
    processPragmasIntoFields(
      (sourceFile as {}) as PragmaContext,
      reportPragmaDiagnostic
    );

    sourceFile.statements = parseList(
    ...
    function reportPragmaDiagnostic(
      pos: number,
      end: number,
      diagnostic: DiagnosticMessage
    ) {
      parseDiagnostics.push(
        createFileDiagnostic(sourceFile, pos, end, diagnostic)
      );
    }
  }
  ...
  function addJSDocComment<T extends HasJSDoc>(node: T): T {
    ...
    const jsDoc = mapDefined(
      getJSDocCommentRanges(node, sourceFile.text),
      comment =>
        JSDocParser.parseJSDocComment(
          node,
          comment.pos,
  ...
  function parseErrorAtPosition(
    ...
    if (!lastError || start !== lastError.start) {
      parseDiagnostics.push(
        createFileDiagnostic(sourceFile, start, length, message, arg0)
      );
    }
  ...
  function createNode(kind: SyntaxKind, pos?: number): Node {
    ...
    return isNodeKind(kind) || kind === SyntaxKind.Unknown
      ? new NodeConstructor(kind, p, p)
      : kind === SyntaxKind.Identifier
      ? new IdentifierConstructor(kind, p, p)
  ...
  function createNodeArray<T extends Node>(
    ...
    const array = <MutableNodeArray<T>>(
  ...
  function createIdentifier(
    ...
    if (isIdentifier) {
      ...
      node.escapedText = escapeLeadingUnderscores(
        internIdentifier(scanner.getTokenValue())
      );
      nextToken();
      return finishNode(node);
    }
  ...
  function parsePropertyNameWorker(
    allowComputedPropertyNames: boolean
  ): PropertyName {
    if (
      token() === SyntaxKind.StringLiteral ||
      token() === SyntaxKind.NumericLiteral
  ...
  function parseComputedPropertyName(): ComputedPropertyName {
    ...
    const node = <ComputedPropertyName>(
      createNode(SyntaxKind.ComputedPropertyName)
    );
  ...
  function nextTokenCanFollowDefaultKeyword(): boolean {
    ...
    return (
      ...
      (token() === SyntaxKind.AbstractKeyword &&
        lookAhead(nextTokenIsClassKeywordOnSameLine)) ||
      (token() === SyntaxKind.AsyncKeyword &&
        lookAhead(nextTokenIsFunctionKeywordOnSameLine))
    );
  }
  ...
  function isListElement(
    ...
    switch (parsingContext) {
      ...
      case ParsingContext.EnumMembers:
        ...
        return (
          token() === SyntaxKind.OpenBracketToken || isLiteralPropertyName()
        );
      ...
      case ParsingContext.HeritageClauseElement:
        ...
        if (!inErrorRecovery) {
          ...
          return (
            isIdentifier() && !isHeritageClauseExtendsOrImplementsKeyword()
          );
        }
    ...
    switch (kind) {
      ...
      case ParsingContext.RestProperties:
        ...
        return (
          token() === SyntaxKind.CloseParenToken ||
          token() ===
            SyntaxKind.CloseBracketToken /*|| token === SyntaxKind.OpenBraceToken*/
        );
      case ParsingContext.TypeArguments:
      ...
      case ParsingContext.JsxChildren:
        return (
          token() === SyntaxKind.LessThanToken && lookAhead(nextTokenIsSlash)
        );
      default:
        return false;
    }
  ...
  function isInSomeParsingContext(): boolean {
    for (let kind = 0; kind < ParsingContext.Count; kind++) {
      if (parsingContext & (1 << kind)) {
        if (
          isListElement(kind, /*inErrorRecovery*/ true) ||
  ...
  function parseListElement<T extends Node>(
    parsingContext: ParsingContext,
    parseElement: () => T
  ): T {
    const node = currentNode(parsingContext);
  ...
  function currentNode(parsingContext: ParsingContext): Node | undefined {
    ...
    if (
      nodeIsMissing(node) ||
      node.intersectsChange ||
      containsParseError(node)
    ) {
      return undefined;
    }
      ...
      switch (node.kind) {
        ...
        case SyntaxKind.MethodDeclaration:
          ...
          const nameIsConstructor =
            ...
            methodDeclaration.name.originalKeywordKind ===
  ...
  function parseEntityName(
    ...
    while (parseOptional(SyntaxKind.DotToken)) {
      ...
      entity = createQualifiedName(
        entity,
        parseRightSideOfDot(allowReservedWords)
      );
    }
    return entity;
  }
  ...
  function parseRightSideOfDot(allowIdentifierNames: boolean): Identifier {
    ...
    if (
      ...
      const matchesPattern = lookAhead(
  ...
  function parseTemplateExpression(): TemplateExpression {
    const template = <TemplateExpression>(
      createNode(SyntaxKind.TemplateExpression)
    );

    template.head = parseTemplateHead();
    Debug.assert(
      template.head.kind === SyntaxKind.TemplateHead,
  ...
  function parseLiteralLikeNode(
    ...
    if (node.kind === SyntaxKind.NumericLiteral) {
      (<NumericLiteral>node).numericLiteralFlags =
        scanner.getTokenFlags() & TokenFlags.NumericLiteralFlags;
    }
  ...
  function parseTypeReference(): TypeReferenceNode {
    ...
    if (
      !scanner.hasPrecedingLineBreak() &&
      reScanLessThanToken() === SyntaxKind.LessThanToken
    ) {
      node.typeArguments = parseBracketedList(
  ...
  function typeHasArrowFunctionBlockingParseError(node: TypeNode): boolean {
    switch (node.kind) {
      ...
      case SyntaxKind.ConstructorType: {
        ...
        return (
          isMissingList(parameters) ||
          typeHasArrowFunctionBlockingParseError(type)
        );
      }
      case SyntaxKind.ParenthesizedType:
        return typeHasArrowFunctionBlockingParseError(
          (node as ParenthesizedTypeNode).type
        );
      default:
        return false;
    }
  ...
  function parseJSDocAllType(
    postFixEquals: boolean
  ): JSDocAllType | JSDocOptionalType {
    const result = createNode(SyntaxKind.JSDocAllType) as JSDocAllType;
    if (postFixEquals) {
      return createPostfixType(
        SyntaxKind.JSDocOptionalType,
        result
      ) as JSDocOptionalType;
    } else {
      nextToken();
    }
  ...
  function parseJSDocUnknownOrNullableType():
    ...
    if (
      ...
      const result = <JSDocUnknownType>(
        createNode(SyntaxKind.JSDocUnknownType, pos)
      );
      return finishNode(result);
    } else {
      const result = <JSDocNullableType>(
        createNode(SyntaxKind.JSDocNullableType, pos)
      );
  ...
  function parseJSDocFunctionType(): JSDocFunctionType | TypeReferenceNode {
    if (lookAhead(nextTokenIsOpenParen)) {
      const result = <JSDocFunctionType>(
        createNodeWithJSDoc(SyntaxKind.JSDocFunctionType)
      );
      nextToken();
      fillSignature(
        SyntaxKind.ColonToken,
        SignatureFlags.Type | SignatureFlags.JSDoc,
        result
      );
      return finishNode(result);
    }
  ...
  function parseParameter(): ParameterDeclaration {
    const node = <ParameterDeclaration>(
      createNodeWithJSDoc(SyntaxKind.Parameter)
    );
  ...
  function parseSignatureMember(
    ...
    const node = <CallSignatureDeclaration | ConstructSignatureDeclaration>(
      createNodeWithJSDoc(kind)
    );
  ...
  function isIndexSignature(): boolean {
    return (
      token() === SyntaxKind.OpenBracketToken &&
      lookAhead(isUnambiguouslyIndexSignature)
    );
  }
  ...
  function parseIndexSignatureDeclaration(
    ...
    node.parameters = parseBracketedList(
  ...
  function parsePropertyOrMethodSignature(
    ...
    if (
      ...
      // [Yield] nor [Await]
      fillSignature(SyntaxKind.ColonToken, SignatureFlags.Type, <
        MethodSignature
      >node);
    } else {
  ...
  function parseTypeMember(): TypeElement {
    ...
    if (
      token() === SyntaxKind.NewKeyword &&
      lookAhead(nextTokenIsOpenParenOrLessThan)
    ) {
    ...
    return parsePropertyOrMethodSignature(<PropertySignature | MethodSignature>(
      node
    ));
  }
  ...
  function parseMappedType() {
    ...
    if (
      ...
      node.readonlyToken = parseTokenNode<
        ReadonlyToken | PlusToken | MinusToken
      >();
  ...
  function parseTupleElementType() {
    ...
    if (
      !(contextFlags & NodeFlags.JSDoc) &&
      type.kind === SyntaxKind.JSDocNullableType &&
      type.pos === (<JSDocNullableType>type).type.pos
    ) {
  ...
  function parseTupleType(): TupleTypeNode {
    ...
    node.elementTypes = parseBracketedList(
  ...
  function parseParenthesizedType(): TypeNode {
    const node = <ParenthesizedTypeNode>(
      createNode(SyntaxKind.ParenthesizedType)
    );
  ...
  function parseFunctionOrConstructorType(): TypeNode {
    const pos = getNodePos();
    const kind = parseOptional(SyntaxKind.NewKeyword)
  ...
  function parseLiteralTypeNode(negative?: boolean): LiteralTypeNode {
    ...
    if (negative) {
      unaryMinusExpression = createNode(
        SyntaxKind.PrefixUnaryExpression
      ) as PrefixUnaryExpression;
      unaryMinusExpression.operator = SyntaxKind.MinusToken;
      nextToken();
    }
    let expression: BooleanLiteral | LiteralExpression | PrefixUnaryExpression =
      token() === SyntaxKind.TrueKeyword || token() === SyntaxKind.FalseKeyword
  ...
  function parseImportType(): ImportTypeNode {
    ...
    if (parseOptional(SyntaxKind.DotToken)) {
      node.qualifier = parseEntityName(
        /*allowReservedWords*/ true,
        Diagnostics.Type_expected
      );
    }
  ...
  function parseNonArrayType(): TypeNode {
    switch (token()) {
      ...
      case SyntaxKind.TypeOfKeyword:
        return lookAhead(isStartOfTypeOfImportType)
          ? parseImportType()
          : parseTypeQuery();
      case SyntaxKind.OpenBraceToken:
        return lookAhead(isStartOfMappedType)
          ? parseMappedType()
          : parseTypeLiteral();
      case SyntaxKind.OpenBracketToken:
  ...
  function isStartOfType(inStartOfParameter?: boolean): boolean {
    switch (token()) {
      ...
      case SyntaxKind.MinusToken:
        return (
          !inStartOfParameter && lookAhead(nextTokenIsNumericOrBigIntLiteral)
        );
      case SyntaxKind.OpenParenToken:
        // Only consider '(' the start of a type if followed by ')', '...', an identifier, a modifier,
        // or something that starts a type. We don't want to consider things like '(1)' a type.
        return (
          !inStartOfParameter && lookAhead(isStartOfParenthesizedOrFunctionType)
        );
      default:
        return isIdentifier();
    }
    ...
    while (!scanner.hasPrecedingLineBreak()) {
      switch (token()) {
        ...
        case SyntaxKind.QuestionToken:
          ...
          if (
            !(contextFlags & NodeFlags.JSDoc) &&
            lookAhead(nextTokenIsStartOfType)
          ) {
            return type;
          }
  ...
  function createPostfixType(kind: SyntaxKind, type: TypeNode) {
    nextToken();
    const postfix = createNode(kind, type.pos) as
      | OptionalTypeNode
      | JSDocOptionalType
      | JSDocNonNullableType
      | JSDocNullableType;
  ...
  function parseInferType(): InferTypeNode {
    ...
    const typeParameter = <TypeParameterDeclaration>(
      createNode(SyntaxKind.TypeParameter)
    );
  ...
  function parseTypeOrTypePredicate(): TypeNode {
    const typePredicateVariable =
      isIdentifier() && tryParse(parseTypePredicatePrefix);
    const type = parseType();
    if (typePredicateVariable) {
      const node = <TypePredicateNode>(
        createNode(SyntaxKind.TypePredicate, typePredicateVariable.pos)
      );
  ...
  function parseTypeWorker(noConditionalTypes?: boolean): TypeNode {
    ...
    if (
      ...
      const node = <ConditionalTypeNode>(
        createNode(SyntaxKind.ConditionalType, type.pos)
      );
      node.checkType = type;
  ...
  function parseAssignmentExpressionOrHigher(): Expression {
    ...
    if (
      ...
      isAssignmentOperator(reScanGreaterToken())
    ) {
  ...
  function parseSimpleArrowFunctionExpression(
    ...
    if (asyncModifier) {
      node = <ArrowFunction>(
        createNode(SyntaxKind.ArrowFunction, asyncModifier.pos)
      );
      node.modifiers = asyncModifier;
    } else {
      node = <ArrowFunction>(
        createNode(SyntaxKind.ArrowFunction, identifier.pos)
      );
    }

    const parameter = <ParameterDeclaration>(
      createNode(SyntaxKind.Parameter, identifier.pos)
    );
  ...
  function tryParseParenthesizedArrowFunctionExpression():
    ...
    arrowFunction.equalsGreaterThanToken = parseExpectedToken(
      SyntaxKind.EqualsGreaterThanToken
    );
    arrowFunction.body =
      lastToken === SyntaxKind.EqualsGreaterThanToken ||
  ...
  function tryParseAsyncSimpleArrowFunctionExpression():
    ...
    if (token() === SyntaxKind.AsyncKeyword) {
      if (
        lookAhead(isUnParenthesizedAsyncArrowFunctionWorker) === Tristate.True
      ) {
  ...
  function parseParenthesizedArrowFunctionExpressionHead(
    ...
    // close paren.
    if (
      !fillSignature(SyntaxKind.ColonToken, isAsync, node) &&
      !allowAmbiguity
    ) {
      return undefined;
    }
  ...
  function parseConditionalExpressionRest(leftOperand: Expression): Expression {
    ...
    const node = <ConditionalExpression>(
      createNode(SyntaxKind.ConditionalExpression, leftOperand.pos)
    );
    node.condition = leftOperand;
    node.questionToken = questionToken;
    node.whenTrue = doOutsideOfContext(
  ...
  function makeBinaryExpression(
    ...
    const node = <BinaryExpression>(
      createNode(SyntaxKind.BinaryExpression, left.pos)
    );
    node.left = left;
  ...
  function parsePrefixUnaryExpression() {
    const node = <PrefixUnaryExpression>(
      createNode(SyntaxKind.PrefixUnaryExpression)
    );
  ...
  function parseUpdateExpression(): UpdateExpression {
    ...
    if (
      ...
      const node = <PostfixUnaryExpression>(
        createNode(SyntaxKind.PostfixUnaryExpression, expression.pos)
      );
  ...
  function parseSuperExpression(): MemberExpression {
    ...
    const node = <PropertyAccessExpression>(
      createNode(SyntaxKind.PropertyAccessExpression, expression.pos)
    );
  ...
  function parseJsxElementOrSelfClosingElementOrFragment(
    ...
    if (opening.kind === SyntaxKind.JsxOpeningElement) {
      ...
      if (
        ...
        parseErrorAtRange(
          ...
          getTextOfNodeFromSourceText(sourceText, node.openingElement.tagName)
        );
      }
    ...
    if (inExpressionContext && token() === SyntaxKind.LessThanToken) {
      ...
      if (invalidElement) {
        ...
        const badNode = <BinaryExpression>(
          createNode(SyntaxKind.BinaryExpression, result.pos)
        );
        badNode.end = invalidElement.end;
        badNode.left = result;
        badNode.right = invalidElement;
        badNode.operatorToken = createMissingNode(
          SyntaxKind.CommaToken,
          /*reportAtCurrentPosition*/ false,
          /*diagnosticMessage*/ undefined!
        ); // TODO: GH#18217
        badNode.operatorToken.pos = badNode.operatorToken.end =
          badNode.right.pos;
  ...
  function parseJsxAttributes(): JsxAttributes {
    ...
    jsxAttributes.properties = parseList(
  ...
  function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(
    ...
    if (token() === SyntaxKind.GreaterThanToken) {
      ...
      const node: JsxOpeningFragment = <JsxOpeningFragment>(
        createNode(SyntaxKind.JsxOpeningFragment, fullStart)
      );
      scanJsxText();
      return finishNode(node);
    }
    ...
    if (token() === SyntaxKind.GreaterThanToken) {
      ...
      node = <JsxOpeningElement>(
        createNode(SyntaxKind.JsxOpeningElement, fullStart)
      );
      scanJsxText();
    } else {
      ...
      node = <JsxSelfClosingElement>(
        createNode(SyntaxKind.JsxSelfClosingElement, fullStart)
      );
    }
  ...
  function parseJsxElementName(): JsxTagNameExpression {
    ...
    let expression: JsxTagNameExpression =
      token() === SyntaxKind.ThisKeyword
        ? parseTokenNode<ThisExpression>()
        : parseIdentifierName();
    while (parseOptional(SyntaxKind.DotToken)) {
      const propertyAccess: JsxTagNamePropertyAccess = <
        JsxTagNamePropertyAccess
      >createNode(SyntaxKind.PropertyAccessExpression, expression.pos);
  ...
  function parseMemberExpressionRest(
    ...
    while (true) {
      ...
      if (dotToken) {
        const propertyAccess = <PropertyAccessExpression>(
          createNode(SyntaxKind.PropertyAccessExpression, expression.pos)
        );
        propertyAccess.expression = expression;
      ...
      if (
        ...
        nextToken();
        const nonNullExpression = <NonNullExpression>(
          createNode(SyntaxKind.NonNullExpression, expression.pos)
        );
      ...
      if (!inDecoratorContext() && parseOptional(SyntaxKind.OpenBracketToken)) {
        const indexedAccess = <ElementAccessExpression>(
          createNode(SyntaxKind.ElementAccessExpression, expression.pos)
        );
  ...
  function parseTaggedTemplateRest(
    ...
    const tagExpression = <TaggedTemplateExpression>(
      createNode(SyntaxKind.TaggedTemplateExpression, tag.pos)
    );
    ...
    while (true) {
      ...
      if (
        ...
        const callExpr = <CallExpression>(
          createNode(SyntaxKind.CallExpression, expression.pos)
        );
  ...
  function parseTypeArgumentsInExpression() {
    ...
    const typeArguments = parseDelimitedList(
      ParsingContext.TypeArguments,
      parseType
    );
  ...
  function parseParenthesizedExpression(): ParenthesizedExpression {
    const node = <ParenthesizedExpression>(
      createNodeWithJSDoc(SyntaxKind.ParenthesizedExpression)
    );
    parseExpected(SyntaxKind.OpenParenToken);
    node.expression = allowInAnd(parseExpression);
    parseExpected(SyntaxKind.CloseParenToken);
  ...
  function parseArrayLiteralExpression(): ArrayLiteralExpression {
    ...
    node.elements = parseDelimitedList(
  ...
  function parseObjectLiteralElement(): ObjectLiteralElementLike {
    const node = <ObjectLiteralElementLike>(
      createNodeWithJSDoc(SyntaxKind.Unknown)
    );
    ...
    (<MethodDeclaration>node).questionToken = parseOptionalToken(
      SyntaxKind.QuestionToken
    );
    (<MethodDeclaration>node).exclamationToken = parseOptionalToken(
    ...
    if (isShorthandPropertyAssignment) {
      ...
      if (equalsToken) {
        ...
        (<ShorthandPropertyAssignment>(
          node
        )).objectAssignmentInitializer = allowInAnd(
          parseAssignmentExpressionOrHigher
        );
      }
    } else {
      ...
      parseExpected(SyntaxKind.ColonToken);
      (<PropertyAssignment>node).initializer = allowInAnd(
        parseAssignmentExpressionOrHigher
      );
    }
    return finishNode(node);
  ...
  function parseObjectLiteralExpression(): ObjectLiteralExpression {
    const node = <ObjectLiteralExpression>(
    ...
    node.properties = parseDelimitedList(
  ...
  function parseFunctionExpression(): FunctionExpression {
    ...
    const node = <FunctionExpression>(
      createNodeWithJSDoc(SyntaxKind.FunctionExpression)
    );
    node.modifiers = parseModifiers();
  ...
  function parseBlock(
    ...
    if (
      parseExpected(SyntaxKind.OpenBraceToken, diagnosticMessage) ||
  ...
  function parseIfStatement(): IfStatement {
    ...
    node.elseStatement = parseOptional(SyntaxKind.ElseKeyword)
      ? parseStatement()
      : undefined;
    return finishNode(node);
  }
  ...
  function parseForOrForInOrForOfStatement(): Statement {
    ...
    if (token() !== SyntaxKind.SemicolonToken) {
      if (
        ...
        initializer = parseVariableDeclarationList(
          /*inForStatementInitializer*/ true
        );
      } else {
    ...
    if (
      awaitToken
        ? parseExpected(SyntaxKind.OfKeyword)
        : parseOptional(SyntaxKind.OfKeyword)
    ) {
      const forOfStatement = <ForOfStatement>(
        createNode(SyntaxKind.ForOfStatement, pos)
      );
      ...
      const forInStatement = <ForInStatement>(
        createNode(SyntaxKind.ForInStatement, pos)
      );
      ...
      const forStatement = <ForStatement>(
        createNode(SyntaxKind.ForStatement, pos)
      );
  ...
  function parseWithStatement(): WithStatement {
    ...
    node.expression = allowInAnd(parseExpression);
    parseExpected(SyntaxKind.CloseParenToken);
    node.statement = doInsideOfContext(
      NodeFlags.InWithStatement,
      parseStatement
    );
    return finishNode(node);
  }
  ...
  function parseThrowStatement(): ThrowStatement {
    ...
    node.expression = scanner.hasPrecedingLineBreak()
      ? undefined
  ...
  function parseTryStatement(): TryStatement {
    ...
    node.catchClause =
      token() === SyntaxKind.CatchKeyword ? parseCatchClause() : undefined;
  ...
  function parseExpressionOrLabeledStatement():
    ...
    // a colon.
    const node = <ExpressionStatement | LabeledStatement>(
      createNodeWithJSDoc(SyntaxKind.Unknown)
    );
  ...
  function nextTokenIsIdentifierOrKeywordOnSameLine() {
    nextToken();
    return (
      tokenIsIdentifierOrKeyword(token()) && !scanner.hasPrecedingLineBreak()
    );
  }
  ...
  function nextTokenIsFunctionKeywordOnSameLine() {
    nextToken();
    return (
      token() === SyntaxKind.FunctionKeyword && !scanner.hasPrecedingLineBreak()
    );
  }
  ...
  function parseStatement(): Statement {
    switch (token()) {
      ...
      case SyntaxKind.VarKeyword:
        return parseVariableStatement(<VariableStatement>(
          createNodeWithJSDoc(SyntaxKind.VariableDeclaration)
        ));
      case SyntaxKind.LetKeyword:
        if (isLetDeclaration()) {
          return parseVariableStatement(<VariableStatement>(
      ...
      case SyntaxKind.ClassKeyword:
        return parseClassDeclaration(<ClassDeclaration>(
          createNodeWithJSDoc(SyntaxKind.ClassDeclaration)
        ));
      case SyntaxKind.IfKeyword:
    ...
    switch (token()) {
      ...
      case SyntaxKind.ExportKeyword:
        ...
        switch (token()) {
          ...
          case SyntaxKind.AsKeyword:
            return parseNamespaceExportDeclaration(<NamespaceExportDeclaration>(
              node
            ));
          default:
      ...
      default:
        if (node.decorators || node.modifiers) {
          ...
          const missing = createMissingNode<Statement>(
  ...
  function parseObjectBindingPattern(): ObjectBindingPattern {
    const node = <ObjectBindingPattern>(
      createNode(SyntaxKind.ObjectBindingPattern)
    );
    parseExpected(SyntaxKind.OpenBraceToken);
    node.elements = parseDelimitedList(
  ...
  function parseArrayBindingPattern(): ArrayBindingPattern {
    const node = <ArrayBindingPattern>(
  ...
  function parseVariableDeclarationList(
    ...
    const node = <VariableDeclarationList>(
      createNode(SyntaxKind.VariableDeclarationList)
    );
    ...
    if (
      ...
      node.declarations = parseDelimitedList(
  ...
  function canFollowContextualOfKeyword(): boolean {
    return (
      nextTokenIsIdentifier() && nextToken() === SyntaxKind.CloseParenToken
    );
  }
  ...
  function parseDecorators(): NodeArray<Decorator> | undefined {
    ...
    while (true) {
      ...
      decorator.expression = doInDecoratorContext(
  ...
  function parseModifiers(
    ...
    while (true) {
      ...
      const modifier = finishNode(<Modifier>(
        createNode(modifierKind, modifierStart)
      ));
  ...
  function parseClassElement(): ClassElement {
    if (token() === SyntaxKind.SemicolonToken) {
      const result = <SemicolonClassElement>(
        createNode(SyntaxKind.SemicolonClassElement)
      );
      nextToken();
      return finishNode(result);
    }
    ...
    if (
      ...
      return parsePropertyOrMethodDeclaration(<
        PropertyDeclaration | MethodDeclaration
      >node);
    }
    ...
    if (node.decorators || node.modifiers) {
      ...
      node.name = createMissingNode<Identifier>(
        SyntaxKind.Identifier,
        /*reportAtCurrentPosition*/ true,
        Diagnostics.Declaration_expected
      );
  ...
  function parseClassExpression(): ClassExpression {
    return <ClassExpression>(
      parseClassDeclarationOrExpression(
        <ClassLikeDeclaration>createNodeWithJSDoc(SyntaxKind.Unknown),
  ...
  function parseClassDeclaration(node: ClassLikeDeclaration): ClassDeclaration {
    return <ClassDeclaration>(
      parseClassDeclarationOrExpression(node, SyntaxKind.ClassDeclaration)
    );
  }
  ...
  function parseHeritageClause(): HeritageClause {
    const tok = token();
    Debug.assert(
      tok === SyntaxKind.ExtendsKeyword || tok === SyntaxKind.ImplementsKeyword
  ...
  function parseExpressionWithTypeArguments(): ExpressionWithTypeArguments {
    const node = <ExpressionWithTypeArguments>(
      createNode(SyntaxKind.ExpressionWithTypeArguments)
    );
  ...
  function parseEnumDeclaration(node: EnumDeclaration): EnumDeclaration {
    ...
    node.name = parseIdentifier();
    if (parseExpected(SyntaxKind.OpenBraceToken)) {
      node.members = parseDelimitedList(
    ...
    node.body = parseOptional(SyntaxKind.DotToken)
      ? <NamespaceDeclaration>(
          parseModuleOrNamespaceDeclaration(
            <ModuleDeclaration>createNode(SyntaxKind.Unknown),
  ...
  function isExternalModuleReference() {
    return (
      token() === SyntaxKind.RequireKeyword && lookAhead(nextTokenIsOpenParen)
    );
  }
  ...
  function parseImportDeclarationOrImportEqualsDeclaration(
    ...
    if (
      ...
      // import {
      (<ImportDeclaration>node).importClause = parseImportClause(
        identifier,
        afterImportPos
      );
  ...
  function parseImportClause(
    ...
    const importClause = <ImportClause>(
      createNode(SyntaxKind.ImportClause, fullStart)
    );
    if (identifier) {
      // ImportedDefaultBinding:
      //  ImportedBinding
  ...
  function parseExternalModuleReference() {
    const node = <ExternalModuleReference>(
      createNode(SyntaxKind.ExternalModuleReference)
    );
  ...
  function parseNamespaceImport(): NamespaceImport {
    ...
    const namespaceImport = <NamespaceImport>(
      createNode(SyntaxKind.NamespaceImport)
    );
  ...
  function walkTreeForExternalModuleIndicators(node: Node): Node | undefined {
    return isImportMeta(node)
      ? node
      : forEachChild(node, walkTreeForExternalModuleIndicators);
  }
  ...
  export namespace JSDocParser {
    export function parseJSDocTypeExpressionForTests(
      ...
      return jsDocTypeExpression
        ? { jsDocTypeExpression, diagnostics }
        : undefined;
    }

    // Parses out a JSDoc type expression.
    export function parseJSDocTypeExpression(
      mayOmitBraces?: boolean
    ): JSDocTypeExpression {
      const result = <JSDocTypeExpression>(
        createNode(SyntaxKind.JSDocTypeExpression)
      );
    ...
    export function parseJSDocCommentWorker(
      ...
      function parseTag(indent: number) {
        ...
        switch (tagName.escapedText) {
          ...
          case "argument":
          case "param":
            return parseParameterOrPropertyTag(
              start,
              tagName,
          ...
          switch (tok) {
            ...
            case SyntaxKind.OpenBraceToken:
              ...
              if (
                lookAhead(
                  () =>
                    nextJSDocToken() === SyntaxKind.AtToken &&
                    tokenIsIdentifierOrKeyword(nextJSDocToken()) &&
                    scanner.getTokenText() === "link"
                )
              ) {
                pushComment(scanner.getTokenText());
                nextJSDocToken();
      ...
      function parseBracketNameInPropertyAndParamTag(): {
        ...
        if (token() === SyntaxKind.NoSubstitutionTemplateLiteral) {
          ...
          return {
            name: createIdentifier(/*isIdentifier*/ true),
            isBracketed: false
          };
        }
      ...
      function parseParameterOrPropertyTag(
        ...
        const result =
          target === PropertyLikeParse.Property
            ...
            : <JSDocParameterTag>(
                createNode(SyntaxKind.JSDocParameterTag, start)
              );
        const comment = parseTagComments(
          indent + scanner.getStartPos() - start
        );
      ...
      function parseNestedTypeLiteral(
        ...
        if (
          ...
          const typeLiteralExpression = <JSDocTypeExpression>(
            createNode(SyntaxKind.JSDocTypeExpression, scanner.getTokenPos())
          );
          ...
          if (children) {
            jsdocTypeLiteral = <JSDocTypeLiteral>(
              createNode(SyntaxKind.JSDocTypeLiteral, start)
            );
      ...
      function parseReturnTag(
        ...
        const result = <JSDocReturnTag>(
          createNode(SyntaxKind.JSDocReturnTag, start)
        );
        result.tagName = tagName;
      ...
      function parseTypeTag(start: number, tagName: Identifier): JSDocTypeTag {
        ...
        result.tagName = tagName;
        result.typeExpression = parseJSDocTypeExpression(
          /*mayOmitBraces*/ true
        );
        return finishNode(result);
      }
      ...
      function parseAugmentsTag(
        ...
        const result = <JSDocAugmentsTag>(
          createNode(SyntaxKind.JSDocAugmentsTag, start)
        );
        result.tagName = tagName;
      ...
      function parsePropertyAccessEntityNameExpression() {
        let node:
          | Identifier
          | PropertyAccessEntityNameExpression = parseJSDocIdentifierName();
        while (parseOptional(SyntaxKind.DotToken)) {
          const prop: PropertyAccessEntityNameExpression = createNode(
      ...
      function parseTypedefTag(
        ...
        const typedefTag = <JSDocTypedefTag>(
          createNode(SyntaxKind.JSDocTypedefTag, start)
        );
        ...
        if (
          ...
          while ((child = tryParse(() => parseChildPropertyTag(indent)))) {
            ...
            if (child.kind === SyntaxKind.JSDocTypeTag) {
              ...
              jsdocTypeLiteral.jsDocPropertyTags = append(
                jsdocTypeLiteral.jsDocPropertyTags as MutableNodeArray<
                  JSDocPropertyTag
                >,
                child
              );
          ...
          if (jsdocTypeLiteral) {
            ...
            typedefTag.typeExpression =
              ...
              !isObjectOrObjectArrayTypeReference(
                childTypeTag.typeExpression.type
              )
                ? childTypeTag.typeExpression
        ...
        return finishNode(
          typedefTag,
          end || typedefTag.comment !== undefined
            ? scanner.getStartPos()
            : (
                typedefTag.fullName ||
                typedefTag.typeExpression ||
      ...
      function parseJSDocTypeNameWithNamespace(nested?: boolean) {
        ...
        if (parseOptional(SyntaxKind.DotToken)) {
          const jsDocNamespaceNode = <JSDocNamespaceDeclaration>(
            createNode(SyntaxKind.ModuleDeclaration, pos)
          );
          if (nested) {
            jsDocNamespaceNode.flags |= NodeFlags.NestedNamespace;
          }
          jsDocNamespaceNode.name = typeNameOrNamespaceName;
          jsDocNamespaceNode.body = parseJSDocTypeNameWithNamespace(
            /*nested*/ true
          );
      ...
      function parseCallbackTag(
        ...
        const jsdocSignature = createNode(
        ...
        while (
          ...
          jsdocSignature.parameters = append(
            jsdocSignature.parameters as MutableNodeArray<JSDocParameterTag>,
            child
          );
        }
      ...
      function parseChildPropertyTag(indent: number) {
        return parseChildParameterOrPropertyTag(
          PropertyLikeParse.Property,
          indent
        ) as JSDocTypeTag | JSDocPropertyTag | false;
      }
      ...
      function parseChildParameterOrPropertyTag(
        ...
        indent: number,
        name?: EntityName
      ): JSDocTypeTag | JSDocPropertyTag | JSDocParameterTag | false {
              ...
              if (canParseTag) {
                ...
                if (
                  ...
                  (ts.isIdentifier(child.name) ||
                    !escapedTextsEqual(name, child.name.left))
                ) {
                  return false;
                }
                return child;
      ...
      function tryParseChildTag(
        ...
        switch (tagName.escapedText) {
          ...
          case "param":
            t =
              PropertyLikeParse.Parameter | PropertyLikeParse.CallbackParameter;
            break;
          default:
            return false;
        }
      ...
      function parseTemplateTag(
        ...
        const result = <JSDocTemplateTag>(
          createNode(SyntaxKind.JSDocTemplateTag, start)
        );
        result.tagName = tagName;
        result.constraint = constraint;
        result.typeParameters = createNodeArray(
          typeParameters,
          typeParametersPos
        );
        finishNode(result);
...
namespace IncrementalParser {
  export function updateSourceFile(
    ...
    Debug.assert(
      textSpanEnd(changeRange.span) === textSpanEnd(textChangeRange.span)
    );
    Debug.assert(
    ...
    const delta =
      textChangeRangeNewSpan(changeRange).length - changeRange.span.length;
  ...
  function extendToAffectedRange(
    ...
    for (let i = 0; start > 0 && i <= maxLookahead; i++) {
      const nearestNode = findNearestNodeStartingBeforeOrAtPosition(
        sourceFile,
        start
      );
    ...
    const finalSpan = createTextSpanFromBounds(
      start,
      textSpanEnd(changeRange.span)
    );
    const finalLength =
      changeRange.newLength + (changeRange.span.start - start);
  ...
  function checkChangeRange(
    ...
    if (textChangeRange) {
      ...
      if (
        aggressiveChecks ||
        Debug.shouldAssert(AssertionLevel.VeryAggressive)
      ) {
        const oldTextPrefix = oldText.substr(0, textChangeRange.span.start);
        const newTextPrefix = newText.substr(0, textChangeRange.span.start);
        Debug.assert(oldTextPrefix === newTextPrefix);

        const oldTextSuffix = oldText.substring(
          textSpanEnd(textChangeRange.span),
          oldText.length
        );
  ...
  interface IncrementalNodeArray
    extends NodeArray<IncrementalNode>,
      IncrementalElement {
    length: number;
  }
      ...
      currentNode(position: number) {
        ...
        if (position !== lastQueriedPosition) {
          ...
          if (
            ...
            currentArrayIndex < currentArray.length - 1
          ) {
            currentArrayIndex++;
...
/*@internal*/
export function processCommentPragmas(
    ...
    switch (key) {
      ...
      case "amd-dependency": {
        context.amdDependencies = map(
          toArray(entryOrList),
          (x: PragmaPseudoMap["amd-dependency"]) => ({
...
export function tagNamesAreEquivalent(
  ...
  return (
    (<PropertyAccessExpression>lhs).name.escapedText ===
      (<PropertyAccessExpression>rhs).name.escapedText &&
    tagNamesAreEquivalent(
      (<PropertyAccessExpression>lhs).expression as JsxTagNameExpression,
      (<PropertyAccessExpression>rhs).expression as JsxTagNameExpression
    )
  );
}
